...
module.exports = {
	...
	beforecreate:function(req,res){
		...
		serviceController.get(function(objService){
			...
			}else{return res.render('userCreation', {err : objService.err});}
		})
	},
	...
	create:function(req,res){
		...
		console.log(toto);
		Validation.userCreation(toto,function(isOk){
			if(isOk.isOk){
		utilisateurController.create(req,function(obj){
				if(obj.status || typeof obj.utilisateur === 'undefined'){
						...
						if(objConnexion.status){
							compteController.create(obj.utilisateur.id,function(objCompte){
								if(objCompte.status){
									...
									res.redirect('/user');
									emailController.sendEmail("createLogin",objConnexion.login,obj.utilisateur.mail,"Bienvenue");
									return;
								}else{
	...
	get:function(req,res){
		utilisateurController.get(function(obj){
			if(obj.status){
				...
				delete req.session.error;
				delete req.session.success;
				res.view('userGestion',{utilisateurs: obj.utilisateurs, messageError: messageError, messageSuccess : messageSuccess, title: 'Utilisateur' });
				return;
			}
			return res.view('userGestion',{utilisateurs: [], err: obj.err});
		})
	},
};
